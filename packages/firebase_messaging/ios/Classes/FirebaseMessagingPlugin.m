...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if ([@"requestNotificationPermissions" isEqualToString:method]) {
        ...
        @try {
            NSDictionary *arguments = call.arguments;
            setUpCallbackHandle = [arguments[@"setupHandle"] longValue];
            backgroundMessageHandle = [arguments[@"backgroundHandle"] longValue];
            //long handle = [call.arguments[0] longValue];
            [self saveCallbackHandle:backgroundSetupCallback handle:setUpCallbackHandle];
            [self saveCallbackHandle:backgroundMessageCallback handle:backgroundMessageHandle];
         }
        @catch(NSException *exception) {
            NSLog(@"%@", exception.reason);
            NSLog(@"IOS: There was an exception when getting callback handle from Dart side");
        }
        result(@(YES));
...
fetchCompletionHandler:(void (^)(UIBackgroundFetchResult result))completionHandler {
    ...
    /*
        Uncomment the below if you want to handle the background message even if the App is in Forground.
        For e.g: Messages having "content-available = 1" (Silent Push).
    */ 
...
- (void) queueMethodCall:(NSString *) method callbackName:(NSString*)callback arguments:(NSDictionary*)arguments {
    ...
    id handleValue = [_userDefaults objectForKey:callback];
    NSMutableDictionary *messageData = [[NSMutableDictionary alloc] init];
    [messageData setValue:handleValue forKey:@"handle"];
    [messageData setValue:arguments forKey:@"message"];
    
    @synchronized(self) {
        if (initialized) {
            [self invokeMethod:method callbackHandle:handle arguments:messageData];
        } else {
            NSArray *call = @[method, @(handle), messageData];
...
- (void) invokeMethod:(NSString *) method callbackHandle:(long)handle arguments:(NSDictionary*)arguments {
    ...
    [_backgroundChannel invokeMethod:method arguments:arguments result:^(id  _Nullable result) {
